            ...
            <div className="w-full max-w-md bg-[#0a0f1e] border border-white/10 rounded-[2.5rem] p-8 relative animate-in zoom-in-95 duration-300 shadow-[0_0_50px_rgba(99,102,241,0.15)]">
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input
                                ...
                                value={fields.flatNo} onChange={handleChange}
                                placeholder="A-101"
                    ...
                    <div>
                        ...
                        <input
                            type="text" name="area"
                            value={fields.area} onChange={handleChange}
                ...
                <div className="flex gap-3 mt-8">
                    <button
                        onClick={onClose}
                        className="flex-1 py-3.5 border border-white/10 text-gray-400 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:text-white hover:border-white/30 transition-all"
                    >
                        Cancel
                    </button>
                    <button
                        onClick={handleSave}
                        className="flex-1 py-3.5 bg-white text-black rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 hover:scale-[1.02] transition-all"
                    >
...
const Signup = () => {
    ...
    const getAddress = async (lat, lng) => {
        try {
            ...
            // Extract parts from Nominatim response
    ...
    const handleGetLocation = () => {
        ...
        const timeoutId = setTimeout(async () => {
            if (!locationReceivedRef.current) {
                ...
                setFormData(prev => ({
                    ...prev, lat: defaultLat, lng: defaultLng,
                    address: fullAddress, addressFields: fields
                }));
                setLocationLoading(false);
                toast("Default location set — tap Edit to update your address! 📍", { icon: 'ℹ️' });
            }
        }, 6000);
        ...
        if (!navigator.geolocation) {
            ...
            setLocationLoading(false);
            setError("Geolocation not supported. Please edit your address manually.");
            // Set default
        ...
        navigator.geolocation.getCurrentPosition(
            ...
            async () => {
                ...
                setLocationLoading(false);
                toast("Location denied. Default set — tap Edit to update! 📍", { icon: 'ℹ️' });
            },
    ...
    const toggleRole = (selectedRole) => {
        setFormData(prev => ({
            ...prev,
            role:         selectedRole,
            businessName: selectedRole === "customer" ? "" : prev.businessName
        }));
        setError("");
    };
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData.lat || !formData.lng) {
            setError("Please sync your location first!");
            return;
        }
    ...
    return (
        <>
            <div className="min-h-screen flex items-center justify-center bg-[#020617] px-4 py-20 relative overflow-hidden">
                ...
                <div className="w-full max-w-lg bg-white/5 backdrop-blur-2xl p-8 md:p-12 rounded-[3rem] shadow-[0_0_50px_rgba(79,70,229,0.1)] border border-white/10 relative z-10">
                    ...
                    <div className="flex bg-black/40 p-1.5 rounded-2xl mb-6 border border-white/5 shadow-inner">
                        {["customer", "vendor"].map(r => (
                            <button
                                ...
                                className={`flex-1 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-500 ${
                                    formData.role === r
                    ...
                    <form className="space-y-4" onSubmit={handleSubmit} autoComplete="off">
                        <div className="space-y-3">
                            ...
                            <div className="pt-2">
                                <button type="button" onClick={handleGetLocation} disabled={locationLoading}
                                    className={`w-full py-4 rounded-2xl border flex items-center justify-center gap-3 transition-all duration-500 group ${
                                        formData.lat
                                            ? 'bg-green-500/10 border-green-500/50 text-green-400'
                                            : 'bg-veny-primary/10 border-veny-primary/30 text-veny-primary hover:bg-veny-primary/20'
                                    }`}
                                >
                                    {locationLoading
                                ...
                                {formData.address && (
                                    <div className="mt-3 bg-white/[0.03] border border-white/10 rounded-2xl p-4 flex items-start justify-between gap-3">
                                        <div className="flex items-start gap-2 flex-1 min-w-0">
                                            ...
                                            <p className="text-[11px] text-gray-400 font-medium leading-relaxed truncate">
                                                {formData.address}
                                            </p>
                                        </div>
                                ...
                                {/* If no location yet — manual entry option */}
                                {!formData.lat && !locationLoading && (
                                    <button
                                        type="button"
                                        onClick={() => {
                                            // Set dummy coords so modal can open
                                            setFormData(prev => ({
                                                ...prev, lat: 28.5355, lng: 77.3910
                                            }));
                        ...
                        <div className="pt-4">
                            <Button
                                text={`Create ${formData.role === 'customer' ? 'Account' : 'Business'}`}
                                loading={loading}
            ...
            {showAddressModal && (
                <AddressModal
                    addressData={formData.addressFields}
